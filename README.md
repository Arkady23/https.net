...
После окончания работы prg скрипта FoxPro9.exe выполняет комады:
```xBase
...
Сервер https.net воспринимает поток POST, если он присутствует, в двух вариантах — как стандартный ввод или как принимаемый произвольный файл, в зависимости от заголовка "Content-Disposition". Если в этом заголовке обнаружен параметр "filename", то весь поток POST помещается в указанный параметром filename файл.
...
#### Вставка файла в исходящий поток
Сервер https.net предусматривает скоростной метод вставки содержимого локального файла в исходящий поток. Файл вставляется минуя буферы памяти непосредственно в трафик ответа. Для этого в строке возврата статуса нужно указать параметр filename, например:
```
200 OK; filename=D:/myFolder/myFile.pdf
```
Передаваемый статус будет 200 OK, а в конец ответа в трафик будет добавлено содержимое файла D:/myFolder/myFile.pdf. Если к файлу нужны какие-то сопровождающие заголовки, например Content-Length, то их должен вставить сам скрипт.
### Обсуждение
Задать вопрос или обсудить тему, касающуюся сервера https.net или VFP/VFPA, вы можете в разделе проекта `Issues` > `New issue`.  

### История версий
В папке https.net всегда можно скачать последнюю рабочую версию https.net.exe.  
...
2.0.3 May 2026. Серьезные изменения во всех модулях. Много быстрей стал движок. Кардинально переделан механизм логирования. Произведено тестирование сервера. Высочайшие показатели для слабого ПК с двумя ядрами и 8Гб ОП! Тест на 50000 запросов на 200 сессиях движка:  
D:\work\https.net>bombardier-windows-amd64.exe -c 200 -n 50000 http://localhost:8080/  
Bombarding http://localhost:8080/ with 50000 request(s) using 200 connection(s)  
 50000 / 50000 [===============================================================] 100.00% 1233/s 40s  
Done!  
Statistics        Avg      Stdev        Max  
  Reqs/sec      1239.08     142.80    2429.22  
  Latency      161.35ms     8.54ms   263.03ms  
  HTTP codes:  
    1xx - 0, 2xx - 49999, 3xx - 0, 4xx - 0, 5xx - 0  
    others - 1  
  Errors:  
    dial tcp [::1]:8080: connectex: No connection could be made because the target machine actively refused it. - 1  
  Throughput:     2.22MB/s  
2.1.0 June 2026. Кардинальные изменения. ИИ gooole.com утверждает, что прямой обмен без прослойки VFP.Memlib32/VFP.Memlib
...
2.3.1 August 2026. Добавлена возможность задавать пароль сертификату, использовать обновление записей AAAA на сервисе Cloudflare в части динамического адреса IPv6, который берется из свойств сети где работает https.net.  
